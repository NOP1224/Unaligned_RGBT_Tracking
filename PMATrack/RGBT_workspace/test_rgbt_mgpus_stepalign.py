...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, save_dir, num_gpu=1, epoch=300, debug=0, script_name='adapter', test_mode = 'all', end_fix='', sid=0, all_seq=0):
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            region, confidence, offset_online, merge_flag = tracker.track_notalign(image_rgb_size, image_rgb_size_unalign, RGB_gt[frame_idx].tolist(), region_gt_tir=T_gt[frame_idx].tolist())  # xywh
...
class RGBT_Track(object):
    ...
    def track_notalign(self, img, img_unalign, region_gt, region_gt_tir):
        ...
        return pred_bbox, pred_score, offset, merge_flag

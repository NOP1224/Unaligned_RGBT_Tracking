...
class CrossAttention2(nn.Module):
    ...
    def forward(self, x_v, x_i, warp_x, warp_xi, mask=None, return_attention=False):
        # x: B, N, C
        # mask: [B, N, ] torch.bool
        
        # temp_v, sear_v = x_v[:, :self.z_size], x_v[:, self.z_size:]
        # temp_i, sear_i = x_i[:, :self.z_size], x_i[:, self.z_size:]
        B, N, C = x_v.shape

        
        q_v = self.q(x_v).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        q_i = self.q(x_i).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        
        k_v = self.k(x_v).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k_i = self.k(x_i).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        
        v_v = self.v(x_v).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v_i = self.v(x_i).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        # k, v = kv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)

        attn_v = (q_v @ k_v.transpose(-2, -1)) * self.scale
        attn_i = (q_i @ k_i.transpose(-2, -1)) * self.scale
        
        if self.rpe:
            relative_position_bias = self.relative_position_bias_table[:, self.relative_position_index].unsqueeze(0)
            attn_v += relative_position_bias
            attn_i += relative_position_bias
        if mask is not None:
            attn_v = attn_v.masked_fill(mask.unsqueeze(1).unsqueeze(2), float('-inf'),)
            attn_i = attn_i.masked_fill(mask.unsqueeze(1).unsqueeze(2), float('-inf'),)
            
        attn_v = attn_v.softmax(dim=-1)
        attn_v = self.attn_drop(attn_v)
        attn_i = attn_v.softmax(dim=-1)
        attn_v = self.attn_drop(attn_v)
        
        a_v = (attn_i @ v_v).transpose(1, 2).reshape(B, N, C)
        a_i = (attn_v @ v_i).transpose(1, 2).reshape(B, N, C)
        
        a_v = self.proj(a_v)
        a_v = self.proj_drop(a_v)
        
        a_i = self.proj(a_i)
        a_i = self.proj_drop(a_i)
        
        if return_attention:
            return a_v, a_i, attn_v, attn_i
        else:
            return a_v, a_i

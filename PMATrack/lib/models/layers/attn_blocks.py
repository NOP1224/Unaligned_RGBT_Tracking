...
from lib.models.layers.attn import Attention, CrossAttention2
...
class CABlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn= CrossAttention2(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
    ...
    def forward(self, x, xi, mask=None):
        attn = self.drop_path(self.attn(self.norm1(x), self.norm1(xi), mask))
        x = x + attn
        xi = xi + attn
        x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class CABlock_Warp(nn.Module):
    ...
    def forward(self, x, xi, warp_x, warp_xi, mask=None):
        attn, attn_i = self.drop_path(self.attn(self.norm1(x), self.norm1(xi), self.norm1(warp_x), self.norm1(warp_xi), mask))
        x = x + attn
        xi = xi + attn_i
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        xi = xi + self.drop_path(self.mlp(self.norm2(xi)))

...
class OSTrack_twobranch(nn.Module):
    ...
    def forward(self, template: torch.Tensor,
        ...
        x, aux_dict = self.backbone(z=template, x=search,
                                    ...
                                    return_last_attn=return_last_attn, offset_gt=offset_gt)
        # Forward head
        feat_last = x
...
def build_ostrack(cfg, training=True):
    ...
    if training and ('OSTrack' in cfg.MODEL.PRETRAIN_FILE or 'DropTrack' in cfg.MODEL.PRETRAIN_FILE):
        ...
        if 'DropTrack' in cfg.MODEL.PRETRAIN_FILE:
            print("Using The Weights of DropMAE")
            for k,v in checkpoint["net"].items():
                if k in ['box_head.conv1_ctr.0.weight','box_head.conv1_offset.0.weight','box_head.conv1_size.0.weight']:
                    # v = torch.cat([v,v],1)
                    v = v
                # elif k in ['box_head.conv1_ctr.0.weight','box_head.conv1_offset.0.weight','box_head.conv1_size.0.weight']:
                #     v = torch.cat([v,v],1)
        ...
        stage1_model = cfg.MODEL.STAGE1_FILE
        stage1_ckpt = torch.load(stage1_model, map_location="cpu")
        missing_keys, unexpected_keys = model.load_state_dict(stage1_ckpt["net"], strict=False)
        print('Load pretrained model from: ' + stage1_model)
        
        for key in missing_keys:
            print(f"Missing Keys: ----{key}----")
            
        for key in unexpected_keys:    
            print(f"Unexpected Keys: ----{key}----")

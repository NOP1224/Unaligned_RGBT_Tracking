...
class SynMU_Train(BaseVideoDataset):
    def __init__(self, root=None, split='train', dtype='rgbrgb', image_loader=jpeg4py_loader, data_fraction=None, framelist=1, pre_align=False):
        self.root = env_settings().synmu_train_dir if root is None else root
        super().__init__('SynMU_Trainset', root, image_loader)
        assert split in ['train', 'test'], 'Only support all, train or test split in SynMU_Trainset, got {}'.format(split)
    ...
    def get_name(self):
        return 'SynMU_Trainset'
    
    def _get_sequence_list(self, split):
        ltr_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..')
        # file_path = os.path.join(ltr_path, 'data_specs', 'mydataset_training_list.txt')
        if split == 'train':
            file_path = os.path.join(ltr_path, 'data_specs', 'synmu_all.txt')
        if split == 'test':
            file_path = os.path.join(ltr_path, 'data_specs', 'synmu_val.txt')
    ...
    def get_sequence_info(self, seq_id):
        ...
        valid = (bbox_v[:, 2] < 2000) & (bbox_v[:, 2] > 10) & \
                (bbox_v[:, 3] < 2000) & (bbox_v[:, 3] > 10) & \
                (bbox_v[:, 0] < 2000) & (bbox_v[:, 0] > 0)  & \
                (bbox_v[:, 1] < 2000) & (bbox_v[:, 1] > 0)  & \
                (bbox_i[:, 2] < 2000) & (bbox_i[:, 2] > 10) & \
                (bbox_i[:, 3] < 2000) & (bbox_i[:, 3] > 10) & \
                (bbox_i[:, 0] < 2000) & (bbox_i[:, 0] > 0)  & \
                (bbox_i[:, 1] < 2000) & (bbox_i[:, 1] > 0)

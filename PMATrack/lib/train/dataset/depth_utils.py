...
def get_x_framev2(color_image, ir_image, ir_align_image):
    ''' read RGB and depth/IR images (兼容路径和ndarray) '''

    # --- 处理 RGB ---
    if isinstance(color_image, str):  # 文件路径
        rgb = cv2.imread(color_image)
        if rgb is not None:
            rgb = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)
    elif isinstance(color_image, np.ndarray):  # 直接是图像
        rgb = color_image
    else:
        rgb = None

    # --- 处理 Depth/IR ---
    if isinstance(ir_image, str):  # 文件路径
        ir = cv2.imread(ir_image, -1)
    elif isinstance(ir_image, np.ndarray):
        ir = ir_image
    else:
        ir = None
        
    if isinstance(ir_align_image, str):  # 文件路径
        ir_align = cv2.imread(ir_align_image, -1)
    elif isinstance(ir_align_image, np.ndarray):
        ir_align = ir_align_image
    else:
        ir_align = None
        
    if ir is not None and rgb is not None:
        ir_resized = cv2.resize(ir, (rgb.shape[1], rgb.shape[0]))
        img_rgb_size = cv2.merge((rgb, ir_resized, ir_align))
        rgb_resized = cv2.resize(rgb, (ir.shape[1], ir.shape[0]))
        ir_align_resized = cv2.resize(ir_align, (ir.shape[1], ir.shape[0]))
        img_tir_size = cv2.merge((rgb_resized, ir, ir_align_resized))
        return img_rgb_size, img_tir_size
    else:
        print('No such dtype !!! ')
        img = None
...
def get_x_framev3(color_image, ir_image):
    ''' read RGB and depth/IR images (兼容路径和ndarray) '''
    ...
    if ir is not None and rgb is not None:
        ir_resized = cv2.resize(ir, (rgb.shape[1], rgb.shape[0]))
        img_rgb_size = cv2.merge((rgb, ir_resized))
        rgb_resized = cv2.resize(rgb, (ir.shape[1], ir.shape[0]))
        img_tir_size = cv2.merge((rgb_resized, ir))
        return img_rgb_size, img_tir_size
    else:
        print('No such dtype !!! ')
        img = None

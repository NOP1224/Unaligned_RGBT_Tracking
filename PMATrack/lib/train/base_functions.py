...
from lib.train.dataset import LUART_LMDB
from lib.train.dataset import LasHeR_Unaligned, LasHeR_Unaligned_Test
from lib.train.dataset import MUART244
...
def names2datasets(name_list: list, settings, image_loader, min_bias = 0, max_bias = 0, train_phase = None):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full", "GOT10K_official_val", "COCO17", "VID", "TRACKINGNET",
                        "DepthTrack_train", "DepthTrack_val", "LasHeR_all", "LasHeR_train", "LasHeR_val", "VisEvent","LUART_LMDB","LUART_train","LUART_test",
                        "LasHeR_Unaligned", "LasHeR_Unaligned_Test", "SynMU_Train", "SynMU_Test", "MUART244"]
        ...
        if name == "LUART_train":
            datasets.append(LUART_Dataset(settings.env.luart_dir, dtype='rgbrgb_notalign_1', split='train', min_bias=min_bias, max_bias=max_bias,
                                          use_lmdb=settings.env.use_lmdb, lmdb_path=settings.env.luart_lmdb_dir))
        if name == "LUART_LMDB":
            datasets.append(LUART_LMDB(settings.env.luart_dir, dtype='rgbrgb_notalign_1', split='train', min_bias=min_bias, max_bias=max_bias,
                                          use_lmdb=True, lmdb_path=settings.env.luart_lmdb_dir))
        if name == "LUART_test":
            datasets.append(LUART_Dataset(settings.env.luart_dir, dtype='rgbrgb_notalign_1', split='test'))
        if name == "LasHeR_Unaligned":
            datasets.append(LasHeR_Unaligned(settings.env.lasher_unaligned_dir, dtype='rgbrgb_notalign_1'))
        if name == "LasHeR_Unaligned_Test":
            datasets.append(LasHeR_Unaligned_Test(settings.env.lasher_unaligned_dir, dtype='rgbrgb_notalign_1'))
        if name == "MUART244":
            datasets.append(MUART244(settings.env.muart244_dir, dtype='rgbrgb_notalign_1'))
...
def build_dataloaders(cfg, settings):
    ...
    loader_train = LTRLoader('train', dataset_train, training=True, batch_size=cfg.TRAIN.BATCH_SIZE, shuffle=shuffle,
                             num_workers=cfg.TRAIN.NUM_WORKER, drop_last=True, stack_dim=1, sampler=train_sampler, pin_memory=True)
    ...
    if cfg.DATA.VAL.DATASETS_NAME[0] is None:
        ...
        loader_val = LTRLoader('val', dataset_val, training=False, batch_size=cfg.TRAIN.BATCH_SIZE,
                            ...
                            epoch_interval=cfg.TRAIN.VAL_EPOCH_INTERVAL, pin_memory=True)
...
def get_optimizer_scheduler(net, cfg):
    ...
    #     # {"params": [p for n, p in net.named_parameters() if "backbone" not in n and p.requires_grad]},
    #     {
    #         "params": [p for n, p in net.named_parameters() if ("refiner" in n or "neuro" in n or "interaction" in n) and p.requires_grad],
    #         "lr": cfg.TRAIN.LR * cfg.TRAIN.BACKBONE_MULTIPLIER,
    #     },
    # ]
    
    if is_main_process():
        print("Learnable parameters and learning rates are shown below.")
            
        # 遍历 param_dicts 中的每个组别
        for i, param_group in enumerate(param_dicts):
            lr = param_group.get("lr", cfg.TRAIN.LR)  # 如果没有设置 lr，则使用默认的 cfg.TRAIN.LR
            print(f"\nGroup {i+1} (Learning rate: {lr}):")
                
            # 打印该组别的参数名称
            for p in param_group['params']:
                for n, p_named in net.named_parameters():
                    if p is p_named:
                         print(f"  {n} - {p_named.shape}")

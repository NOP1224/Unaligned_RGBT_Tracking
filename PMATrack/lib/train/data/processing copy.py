    ...
    def __call__(self, data: TensorDict, Align_mode = True):
        ...
        if Align_mode == True:
            ...
            if self.transform['joint'] is not None:
                data['template_images'], data['template_anno'], data['template_is_flip_rgb_first'] = self.transform['joint'](
                    image=data['template_images'], bbox=data['template_anno'])
                data['search_images'], data['search_anno'], data['search_is_flip_rgb_first'] = self.transform['joint'](
            ...
            if self.transform['joint'] is not None:
                ...
                # data['search_images_ir'], data['search_anno_ir'], data['search_is_flip_ir_first'] = self.transform['joint'](
            ...
            for tail in ['']:
                for s in ['template', 'search']:
                    ...
                    if s == "template":
                        ...
                        jittered_anno_rgb = [self._get_jittered_box_notalign2(a0,s) for a0,a1 in data[s+'_anno_rgb'+tail]]
                        jittered_anno_dict['rgb'] = jittered_anno_rgb
                    modalities = ['rgb','ir'] if s == "template" else ['rgb']
                    ...
                    for modality in modalities:
                        ...
                        # Apply transforms
                        data[s + '_images_' + modality + tail], data[s + '_anno_' + modality + tail], is_hor_flip = self.transform[s](image=crops, bbox=boxes, joint=False)
                        data[s +'_crop_sz_' + modality + tail] = crop_sz
                        data[s + '_jit_anno_' + modality + tail ] = jittered_anno_dict[modality]
                        # 取当前 transform 的翻转状态
                        curr_flip = int(is_hor_flip[0]) if isinstance(is_hor_flip, (list, torch.Tensor)) else int(is_hor_flip)
                        data[s +'_is_flip_' + modality + tail] = curr_flip

...
class BaseTrainer:
        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch+1, max_epochs+1):
                    ...
                    save_start_epoch = getattr(self.settings, "save_start_epoch", 1)
                    should_save = (
                        (epoch > (max_epochs - save_last_n_epoch) or epoch % save_epoch_interval == 0 )
                        and (epoch >= save_start_epoch or epoch % 5 ==0)
                        and self._checkpoint_dir is not None
                    )
                    if should_save and is_main_process():
                        self.save_checkpoint()
                    if should_save and is_dist_avail_and_initialized():
                        torch.distributed.barrier()
            except:
    ...
    def save_checkpoint(self):
        """Saves a checkpoint of the network and other variables on rank 0."""
        ...
        for key in fields:
            ...
            if key == 'net':
                try:
                    ...
                    raise RuntimeError(
                        'Training checkpoint is incompatible with the current model code. '
                        'Start a new run or resume from a checkpoint produced by this exact version.'
                    ) from exc
            elif key == 'optimizer':

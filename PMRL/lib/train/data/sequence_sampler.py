...
class SequenceSampler(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        template_frames_rgb, tempalte_frames_ir, _,_, template_anno, _, _, _ = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict)
        search_frames_rgb, _, _, _, search_anno, _, meta_obj_test, _ = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)
        template_bbox_rgb = [bbox.numpy() for bbox in template_anno['bbox_rgb']] # tensor -> numpy array
        template_bbox_ir = [bbox.numpy() for bbox in template_anno['bbox_ir']]
        search_bbox = [bbox.numpy() for bbox in search_anno['bbox_rgb']] # tensor -> numpy array
        

        return TensorDict({
                'template_images_rgb': np.array(template_frames_rgb).squeeze(),    # 1 template images
                'template_images_ir': np.array(tempalte_frames_ir).squeeze(),
                'template_annos_rgb': np.array(template_bbox_rgb).squeeze(),
                'template_annos_ir': np.array(template_bbox_ir).squeeze(),
                'search_images': np.array(search_frames_rgb),      # (num_frames) search images
                'search_annos': np.array(search_bbox),
                'seq_id': seq_id,
                'search_class': meta_obj_test.get('object_class_name'),
                'num_frames': len(search_frames_rgb)
                })
...
class Transform:
    ...
    def __call__(self, **inputs):
        ...
        for t in self.transforms:
            ...
            if hasattr(t, 'do_flip'):
                is_hor_flip = t.do_flip
        if len(var_names) == 1:
            return out[var_names[0]]
        # Make sure order is correct
        
        return tuple(list(out[v] for v in var_names) + [is_hor_flip])
...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        ...
        if dims[0] == 6:
            ...
            rgb = image[:3, :, :]
            depth = image[3:6, :, :]
            depth_align = image[6:9, :, :]
            rgb = tvisf.normalize(rgb, self.mean, self.std, self.inplace)
            depth = tvisf.normalize(depth, self.mean, self.std, self.inplace)
            depth_align = tvisf.normalize(depth_align, self.mean, self.std, self.inplace)
            return torch.cat((rgb, depth, depth_align), 0)
        else:
...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:
            ...
            if image.ndim != 3 or image.shape[2] % 3 != 0:
                raise ValueError(f'ToGrayscale expects HxWx(3*k), got shape {image.shape}')
            # RGB-TIR samples are stored as concatenated 3-channel modality
            # groups. Convert each group independently while sharing one random
            # grayscale decision; OpenCV cannot consume a 6-channel image.
            groups = []
            for start in range(0, image.shape[2], 3):
                group = image[:, :, start:start + 3]
                img_gray = cv.cvtColor(group, cv.COLOR_RGB2GRAY)
                groups.append(np.stack([img_gray, img_gray, img_gray], axis=2))
            return np.concatenate(groups, axis=2)
...
class RandomHorizontalFlip(TransformBase):
    ...
    #     return random.random() < self.probability
    def roll(self):
        self.do_flip = random.random() < self.probability  # 保存状态
        return self.do_flip

...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap, bias_min, bias_max,
                 ...
                 train_cls=False, pos_prob=0.5, deterministic=False):
        """
        args:
    ...
    def __getitem__(self, index):
        ...
        # Validation must map each dataset index to a stable sample regardless of
        # worker scheduling or previous random draws. Restore the caller RNG states
        # afterwards so this mode cannot perturb the training sampler.
        py_state = random.getstate()
        np_state = np.random.get_state()
        torch_state = torch.get_rng_state()
        seed = (0x5F3759DF + int(index) * 1000003) & 0xFFFFFFFF
        random.seed(seed)
        np.random.seed(seed)
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            try:
                if True:
                    template_frames_rgb, tempalte_frames_ir, template_anno, meta_obj_train, seq_name = dataset.get_frames(seq_id, template_frame_ids, seq_info_dict)
                    search_frames_rgb, search_frames_ir, search_anno, meta_obj_test, seq_name = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict)
                    RGB_H, RGB_W = search_frames_rgb[0].shape[0], search_frames_rgb[0].shape[1]
                    TIR_H, TIR_W = search_frames_ir[0].shape[0], search_frames_ir[0].shape[1]
                    ...
                    data = TensorDict({'template_images_rgb': template_frames_rgb,
                                        ...
                                        'search_anno_ir': search_anno['bbox_ir_scaled'],
                                        'search_jit_anno_rgb': search_anno.get('search_jit_anno_rgb', None),
                                        ...
                                        'oringin_ir_anno_scaled': search_anno['bbox_ir_scaled'],
                                        'rgb_w': RGB_W,
                                        'rgb_h': RGB_H,
                                        'ir_w': TIR_W,
                                        'ir_h': TIR_H,
                ...
                if self._processing_error_count <= 3:
                    print(
                        f"[TrackingSampler] sample processing failed "
                        f"({self._processing_error_count}/3 shown): {type(exc).__name__}: {exc}"
                    )
                    traceback.print_exc()
                valid = False
        return data

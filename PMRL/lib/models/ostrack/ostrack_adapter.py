...
def build_ostrack(cfg, training=True):
    if cfg.MODEL.PRETRAIN_FILE and ('OSTrack' not in cfg.MODEL.PRETRAIN_FILE) and training:
        pretrained = os.path.abspath(cfg.MODEL.PRETRAIN_FILE)
    else:
        pretrained = ''
    ...
    if cfg.MODEL.BACKBONE.TYPE == 'vit_base_patch16_224_ce_adapter':
        backbone = vit_base_patch16_224_ce_adapter(pretrained, drop_path_rate=cfg.TRAIN.DROP_PATH_RATE,
                                           ...
                                           joint_align_fusion_cfg=dict(cfg.MODEL.JOINT_ALIGN_FUSION),
                                           )
    ...
    if training and ('OSTrack' in cfg.MODEL.PRETRAIN_FILE or 'DropTrack' in cfg.MODEL.PRETRAIN_FILE):
        ...
        if 'DropTrack' in cfg.MODEL.PRETRAIN_FILE:
            ...
            print('Load pretrained model from: ' + cfg.MODEL.PRETRAIN_FILE + "Over !!!")
        else:
            checkpoint = torch.load(cfg.MODEL.PRETRAIN_FILE, map_location="cpu")
            param_dict_rgbt = dict()
            for k,v in checkpoint["net"].items():

...
class NPR_LUART(Metric):
    """Normalized Precision Rate (对齐 Matlab: 0.2 阈值)"""
    def __init__(self, thr=np.linspace(0, 0.5, 51)) -> None:
        super().__init__()
        self.thr = thr
    ...
    def __call__(self, dataset:BaseRGBTDataet, result:TrackerResult, seqs:list):
        ...
        pr = np.array(pr)
        # Matlab 对应 0.2 的点 (索引 20)
        pr_val = pr.mean(axis=0)[5]
        return pr_val, pr


class SR_LUART(Metric):
    """Success Rate (对齐 Matlab: IoU=0.5)"""
    def __init__(self, thr=np.linspace(0, 1, 21)) -> None:
        super().__init__()
        self.thr = thr
...
class PR_LUART(Metric):
    ...
    def __call__(self, dataset:BaseRGBTDataet, result:TrackerResult, seqs:list):
        ...
        for seq_name in seqs:
            ...
            valid = np.ones(len(res), dtype=bool)
            for i in range(len(gt)):
                if np.any(gt[i] <= 0):
                    valid[i] = False
            ...
            pr_cell = []
            for t in self.thr:
                pr_cell.append(np.sum(res[valid] <= t) / (np.sum(valid) + 1e-12))
            pr.append(pr_cell)

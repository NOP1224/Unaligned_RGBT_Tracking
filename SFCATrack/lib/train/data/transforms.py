...
class ToGrayscale(TransformBase):
    ...
    def transform_image(self, image, do_grayscale):
        if do_grayscale:
            ...
            if image.ndim != 3:
                raise ValueError('Expected image with shape HxWxC.')
            channels = image.shape[2]
            if channels == 3:
                img_gray = cv.cvtColor(image, cv.COLOR_RGB2GRAY)
                return np.stack([img_gray, img_gray, img_gray], axis=2)
            if channels == 4:
                img_gray = cv.cvtColor(image, cv.COLOR_RGBA2GRAY)
                return np.stack([img_gray, img_gray, img_gray], axis=2)
            if channels == 6:
                rgb = image[:, :, :3]
                ir = image[:, :, 3:]
                rgb_gray = cv.cvtColor(rgb, cv.COLOR_RGB2GRAY)
                ir_gray = cv.cvtColor(ir, cv.COLOR_RGB2GRAY)
                rgb_gray = np.stack([rgb_gray, rgb_gray, rgb_gray], axis=2)
                ir_gray = np.stack([ir_gray, ir_gray, ir_gray], axis=2)
                return np.concatenate([rgb_gray, ir_gray], axis=2)
            if channels == 1:

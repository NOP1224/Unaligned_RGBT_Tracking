...
class LasHeR(BaseVideoDataset):
    ...
    def _get_sequence_list(self, split):
        ...
        file_path = os.path.join(ltr_path, 'data_specs', f'lasher_{split}.txt')#'lasher_{}.txt'.format(split))
    ...
    def _get_frame_path(self, seq_path, frame_id):
        ...
        rgb_frame_path = os.path.join(seq_path, 'visible', sorted([p for p in os.listdir(os.path.join(seq_path, 'visible')) if os.path.splitext(p)[1] in ['.jpg','.png','.bmp']])[frame_id]) # frames start from 0
        ir_frame_path = os.path.join(seq_path, 'infrared', sorted([p for p in os.listdir(os.path.join(seq_path, 'infrared')) if os.path.splitext(p)[1] in ['.jpg','.png','.bmp']])[frame_id])
